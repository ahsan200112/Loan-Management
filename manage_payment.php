...
$qry = $conn->query("SELECT l.*,concat(b.firstname,' ',b.lastname)as name, b.contact_no, b.email, b.TAXID, b.address from loan_list l inner join borrowers b on b.id = l.borrower_id  order by id asc");
...
<div class="container-fluid">
	<div class="col-lg-12">
		<form id="manage-payment" autocomplete="off">
	 ...
	 $('#manage-payment').submit(function(e){
	 	...
	 	$.ajax({
	 		...
	 		success:function(resp){
	 			if(resp==1){
					$('.modal').modal('hide')
	 				alert_toast("Payment successfully added",'success');
					...
					$('.modal').modal('hide')
					alert_toast("Payment successfully updated",'success')
					setTimeout(function(){
						location.reload()
					},1500)
				}
					...
					$('.modal').modal('hide')
					alert_toast("Unable to add this Payment",'danger')
					setTimeout(function(){
						location.reload()
					},1500)
				}
